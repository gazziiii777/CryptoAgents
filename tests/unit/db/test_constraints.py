...
@pytest.mark.unit
async def test_reopen_same_symbol_after_close(session: AsyncSession) -> None:
    """После закрытия позиции по символу можно открыть новую — записи не конфликтуют.

    Инвариант «одна OPEN-позиция на символ» теперь enforced на repo-уровне
    (read-before-write в PortfolioManager), а не DB-constraint'ом (MariaDB не
    поддерживает partial unique index). Покрытие — test_portfolio_manager::test_dedup.
    """
