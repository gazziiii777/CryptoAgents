# TradingAgents — Architecture (roadmap)

> ⚠️ **Историко-проектный документ (vision/roadmap).** Раздел про persistence написан под SQLite;
> проект переехал на **MariaDB** (драйвер `aiomysql`, `DATABASE_URL` — computed из `DB_*`, бэкап через
> SQLite `.backup()` больше не используется). Актуальное по запуску и БД — в [setup.md](setup.md),
> [architecture.md](architecture.md), [configuration.md](configuration.md). При расхождении источник
> правды — код и эти доки.
