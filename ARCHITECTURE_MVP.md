...
**Этот документ** = минимальный жизнеспособный набор фич для первой работающей версии. Полная роадмапа — в `ARCHITECTURE.md`.

**Цель MVP**: запустить paper trading систему за 1-2 месяца. Меньше фич — **но качество не упрощаем**.

**Цель = НЕ MVP**: всё что в `ARCHITECTURE.md` помечено как "Deferred" или "Iteration 4+".
...
### Iter 2: LLM Analysts + Aggregator (1.5 недели)

- MacroAnalyst → MacroReport (LiteLLM + instructor)
- DerivAndTech merged analyst с tools (get_ohlcv, get_indicators, get_key_levels) → MergedReport
- SentimentAnalyst → SentimentReport
- SimpleAggregator → SignalSynthesis с confluence_score
- LLM cost tracking через success_callback
- Daily budget gate
...
**Тогда** — итеративно подтягивать из `ARCHITECTURE.md`:
...
`ARCHITECTURE.md` — это **2055 строк** полной production-grade спецификации. Этот документ — её **MVP-срез**:
